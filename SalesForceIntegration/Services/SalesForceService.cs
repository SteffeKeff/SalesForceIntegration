...
using SalesForceIntegration.sForce;
using System;
...
namespace SalesForceIntegration.Services
{
    public class SalesForceService
    {
        ...
        /*
        public void updateIsEmailBounced(string contactId)
        {
            var fields = getFields("contact");
            var fieldsQueryBuilder = new StringBuilder();
            ...
            var fieldsQuery = fieldsQueryBuilder.ToString();
            
            string query = string.Format("SELECT IsEmailBounced FROM contact WHERE Id = '{0}'", contactId);


            var queryResult = binding.query(query);
            sObject theContact = queryResult.records[0];
            theContact.Id = contactId;

            bool currentValue = bool.Parse(theContact.Any[0].FirstChild.Value);
            bool newValue = !currentValue;

            theContact.Any[0].FirstChild.Value = newValue.ToString();

            sObject[] toUpdate = new sObject[1];
            toUpdate[0] = theContact;

            var result = binding.update(toUpdate);
        }
        */
    }
}